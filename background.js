...
async function playSound(volume) { const source = 'sound.mp3'; const offscreenData = { command: 'playSound', source, volume }; if (await chrome.offscreen.hasDocument()) { chrome.runtime.sendMessage(offscreenData).catch(e=>{}); return; } await chrome.offscreen.createDocument({ url: 'offscreen.html', reasons: ['AUDIO_PLAYBACK'], justification: 'タイマー終了を通知するため', }); setTimeout(() => { chrome.runtime.sendMessage(offscreenData).catch(e=>{}); }, 100); }